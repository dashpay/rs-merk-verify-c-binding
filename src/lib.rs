...
pub struct ExecuteProofResult {
    ...
    pub elements: *const *mut Element, //sizeof(pointer)
}

#[no_mangle]
pub extern "C" fn execute_proof_c(c_array: *const u8) -> *const ExecuteProofResult {
    let rust_array: &[u8] = unsafe { slice::from_raw_parts(c_array, 32 as usize) };
    let execute_proof_result = execute_proof(rust_array);
    match execute_proof_result {
        Err(_) => ptr::null(),
        Ok((hash, map)) => {
            let elements_map = map.all().map(|key_value_pair| {
